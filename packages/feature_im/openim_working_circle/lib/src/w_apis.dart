...
class WApis {
  ...
  static Future publishMoments({
      ...
      final result = await Future.wait(allMetas.map((e) {
        ...
        OpenIM.iMManager.setUploadFileListener(OnUploadFileListener(
          onUploadProgress: (id, fileSize, streamSize, storageSize) {
            ...
            final progress = streamSize / fileSize;
            final progressTips = '${(progress * 100).toInt()}%';
            final p = (progress).toDouble() / 100.0;

            EasyLoading.showProgress(p, status: progressTips);
          },
    ...
    return HttpUtil.post(
      ...
      data: <String, dynamic>{
        ...
        'permissionGroupIDs': permissionGroupList.map((e) => e.groupID).toList(),
  ...
  static Future commentMoments({
    ...
    return HttpUtil.post(
      ...
      options: Apis.chatTokenOptions,
      data: <String, dynamic>{'workMomentID': workMomentID, 'replyUserID': replyUserID ?? '', 'content': text},
    );
  }
  ...
  static Future deleteComment({
    ...
    return HttpUtil.post(
      WUrls.deleteComment,
      options: Apis.chatTokenOptions,
      data: <String, dynamic>{'workMomentID': workMomentID, 'commentID': commentID},
    );
  }
