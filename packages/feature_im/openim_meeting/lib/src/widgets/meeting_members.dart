...
class _MeetingMembersSheetViewState extends State<MeetingMembersSheetView> {
  ...
  bool get _onlyHostInviteUser => /*_meetingInfo?.onlyHostInviteUser == true*/ false;
  ...
  _muteAll() => widget.controller?.reverse().then((value) =>
      MeetingRepository().operateAllStream(_meetingInfo!.meetingID, DataSp.userID!, microphoneOnEntry: false));

  _unmuteAll() => widget.controller?.reverse().then((value) {
        MeetingRepository().operateAllStream(_meetingInfo!.meetingID, DataSp.userID!, microphoneOnEntry: true);
      });

  // _updateMeetingInfo({
  //       "roomID": _meetingInfo!.meetingID,
  ...
  _allSeeHim(String userID, bool seeHim) async {
    ...
    setState(() {
      if (seeHim) {
        ...
        // _meetingInfo?.beWatchedU?serIDList?.remove(userID);
      }
    });
  ...
  _onTapCamera(String userID, bool value) => MeetingRepository().setPersonalSetting(
        _meetingInfo!.meetingID,
        userID,
        PersonalMeetingSetting(cameraOnEntry: value),
      );

  _onTapMic(String userID, bool value) => MeetingRepository().setPersonalSetting(
        _meetingInfo!.meetingID,
        userID,
        PersonalMeetingSetting(microphoneOnEntry: value),
      );
/*
 ...
 */
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: GestureDetector(
        onTap: () {},
        child: Container(
          constraints: ScreenUtil().orientation == Orientation.portrait
              ? BoxConstraints(maxHeight: 500.h)
              : BoxConstraints(maxHeight: 150.w),
  ...
  Widget _buildItemView(Participant participant) {
    ...
    final isPined = /*_pinedUserIDList.contains(userID)*/ false;
    final isSeeHim = /*_beWatchedUserIDList.contains(userID)*/ false;
