...
class BeCalledView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final inviterUserInfo = memberInfoList?.firstWhereOrNull((e) => e.userID == inviterUserID);
    return Column(
      children: [
        47.verticalSpace,
        Row(
              ...
              child: Column(
                ...
                children: [
                  if (null != inviterUserInfo)
                    sprintf(
                        callType == CallType.audio ? StrRes.whoInvitedVoiceCallHint : StrRes.whoInvitedVideoCallHint,
                        [inviterUserInfo.nickname]).toText
                      ..style = Styles.ts_FFFFFF_17sp_medium,
                  if (null != memberInfoList && memberInfoList!.isNotEmpty)
                    sprintf(StrRes.nPeopleCalling, [memberInfoList!.length]).toText..style = Styles.ts_FFFFFF_14sp,
                ],
              ),
            ),
            16.horizontalSpace,
...
class CallingView extends StatelessWidget {
  ...
  final List<GroupMembersInfo>? membersList;

  @override
  Widget build(BuildContext context) {
    final notInParticipantTracks = membersList != null
        ? membersList!.where(
            (e) => participantTracks.firstWhereOrNull((element) => element.participant.identity == e.userID) == null)
        : <GroupMembersInfo>[];

    return GridView.builder(
      itemCount: notInParticipantTracks.length + participantTracks.length,
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 2,
        mainAxisSpacing: 4.h,
      ...
      itemBuilder: (_, i) {
        if (i < participantTracks.length) {
          return ParticipantWidget.widgetFor(participantTracks.elementAt(i));
        } else {
          final memberInfo = notInParticipantTracks.elementAt(i - participantTracks.length);
          return NoVideoAvatarWidget(
            name: memberInfo.nickname,
            faceURL: memberInfo.faceURL,
            showLoading: memberInfo.userID != OpenIM.iMManager.userID,
          );
        }
      },
    );
