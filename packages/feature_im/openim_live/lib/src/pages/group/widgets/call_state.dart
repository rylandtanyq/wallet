...
abstract class SignalView extends StatefulWidget {
  ...
  final PublishSubject<List<String>>? inviteeUserIDListSubject;
...
abstract class SignalState<T extends SignalView> extends State<T> {
  ...
  List<String> get memberIDList => [widget.inviterUserID, ...widget.inviteeUserIDList];
  ...
  void initState() {
    ...
    widget.inviteeUserIDListSubject?.listen(_inviteeListDidUpdate);
  ...
  void _inviteeListDidUpdate(List<String> list) async {
    if (!mounted) {
      return;
    }
    final reqUserIDs = list.contains(widget.inviterUserID) ? list : [widget.inviterUserID, ...list];
    await widget.onSyncGroupMemberInfo?.call(widget.groupID, reqUserIDs).then(_onUpdateGroupMemberInfo);
  }

  /// 过滤其他房间的信令
  Stream<CallEvent> get sameRoomSignalStream =>
      widget.callEventSubject.stream.where((event) => LiveUtils.isSameRoom(event, widget.roomID));
  ...
  _onStateDidUpdate(CallEvent event) async {
    ...
    if (event.state == CallState.call || event.state == CallState.beCalled || event.state == CallState.calling) {
