...
enum CallingOperationType { participantDisconnected, participantConnected }
...
class LiveUtils {
  ...
  static List<RemoteParticipant> removeObserver(String roomID, Room room) {
    return room.remoteParticipants.values.where((element) {
      Logger.print('removeObserver roomID:$roomID  userID:${element.identity}   ${roomID == element.identity}');
  ...
  static RemoteParticipant? getRemoteParticipant(String? roomID, Room? room) {
    return room?.remoteParticipants.values.where((element) => roomID != element.identity).firstOrNull;
  }
