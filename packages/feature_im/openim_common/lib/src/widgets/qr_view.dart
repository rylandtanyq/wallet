...
class _QrcodeViewState extends State<QrcodeView> with TickerProviderStateMixin {
  ...
  Barcode? result;
  MobileScannerController? controller;  // 改为 MobileScannerController
  ...
  void reassemble() {
    super.reassemble();
    controller?.start();  // 改为 start 方法
  }

  @override
  void initState() {
    _initAnimation();
    // 初始化 MobileScannerController
  ...
  @override
  void dispose() {
    controller?.dispose();  // MobileScanner 的 dispose 方法
    _clearAnimation();
    super.dispose();
  }
  ...
  void _readImage() {
    Permissions.storage(() async {
      ...
      if (null != image) {
        // 创建临时的图片扫描控制器
        final BarcodeCapture? capture = await controller.analyzeImage(image
            .path,formats: [BarcodeFormat.qrCode,BarcodeFormat.code128]);
        if (capture?.barcodes.isNotEmpty ?? false) {
          _parse(capture!.barcodes.first.rawValue);
        } else {
          IMViews.showToast('无法识别二维码');
        }
      }
    });
  }
    ...
    child: Container(
      ...
      child: Row(
        ...
        children: [
          ...
          GestureDetector(
            onTap: () async {
              await controller?.toggleTorch();  // 方法名改为 toggleTorch
              setState(() {});
            },
            child: Container(
              width: 80.w,
              ...
              alignment: Alignment.center,
              child: FutureBuilder(
                future: Future.value(controller?.value.torchState),  // 改为 torchState
                builder: (context, snapshot) {
                  final isTorchOn = snapshot.data == TorchState.on;
                  return isTorchOn ? flashOpen : flashClose;
                },
              ),
            ),
          ),
  ...
  Widget _buildQrView(BuildContext context) {
    return MobileScanner(
      ...
      onDetect: (capture) {  // 扫描回调
        final List<Barcode> barcodes = capture.barcodes;
        if (barcodes.isNotEmpty && mounted) {
          _parse(barcodes.first.rawValue);
        }
      },
    );
  }
