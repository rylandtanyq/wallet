...
class _MediaBrowserState extends State<MediaBrowser>
  ...
  void _preloadImage(int index) {
    ...
    if (0 <= index && index < widget.sources.length) {
      ...
      if (s.file != null && s.file!.existsSync()) {
        imageProvider = FileImage(s.file!);
      } else {
        final url = s.isVideo
            ? s.thumbnail
            : (s.thumbnail.isEmpty ? s.url! : s.thumbnail);
...
class _VideoPlayerViewState extends State<VideoPlayerView> {
  ...
  void initState() {
    super.initState();
    Logger.print(
        'VideoPlayerView initState:[${widget.file}], [${widget.path}], [${widget.url}]',
        fileName: 'video_player_view.dart');
    _playingSubscription = player.stream.playing.listen((event) {
      Logger.print('playing: $event', fileName: 'video_player_view.dart');
      if (event && _showCover && mounted) {
        Logger.print('setShowCover false', fileName: 'video_player_view.dart');
        setState(() {
          _showCover = false;
        });
      }
    ...
    } else if (widget.path?.isNotEmpty == true &&
  ...
  @override
  void dispose() {
    Logger.print('VideoPlayerView dispose', fileName: 'video_player_view.dart');
