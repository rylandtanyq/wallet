...
class HttpUtil {
  ...
  static Future post(
    ...
    try {
      ...
      if (resp.errCode == 0) {
        ...
        if (showErrorToast) {
          final msg = resp.errCode.toString().tr == resp.errCode.toString()
              ? resp.errMsg
              : resp.errCode.toString().tr;
      ...
      if (error is DioException) {
        final errorMsg = '网络连接错误';
        // '接口：$path  信息：${error.message}';
        if (showErrorToast) IMViews.showToast(errorMsg);
        return Future.error(errorMsg);
      }
      final errorMsg = '网络连接错误'; //'接口：$path  信息：${error.toString()}';
      if (showErrorToast) IMViews.showToast(errorMsg);
    ...
    return download(
      ...
      onProgress: (int count, int total) async {
        ...
        if (total == -1) {
          ...
          intervalDo.drop(
              fun: () async {
                saveFileToGallery(File(cachePath),
                    showToast: EasyLoading.isShow);
              },
              milliseconds: 1500);
        }
        if (count == total) {
          saveFileToGallery(File(cachePath), showToast: EasyLoading.isShow);
        }
      },
    );
  }
  ...
  static Future<void> saveFileToGallery(File file,
      {String? name, bool showToast = true, bool isVideo = false}) async {
    Permissions.storage(() async {
      Logger.print('saveFileToGallery: ${file.path}');
      ...
      if (result != null) {
        String tips = StrRes.saveSuccessfully;

        if (Platform.isAndroid && result['filePath'] != null) {
          final filePath = result['filePath'].split('//').last;
          tips = '$tips: $filePath';
        }
